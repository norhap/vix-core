...
class VIXDevicePanelConf(Screen, ConfigListScreen):
    ...
    def saveMypoints(self):
        ...
        message = _('Actualizacion de ubicaciones de montaje...')
        ybox = self.session.openWithCallback(self.delay, MessageBox, message, type=MessageBox.TYPE_INFO, timeout=5, enable_input=False)
        ybox.setTitle(_('Espere por favor.'))

    def delay(self, val):
        message = _('Los cambios necesitan un reinicio del sistema para que surta efecto.\nReiniciar su %s %s ahora?') % (getMachineBrand(), getMachineName())
